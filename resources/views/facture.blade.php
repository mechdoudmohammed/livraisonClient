    ...
    <div class="invoice-box">
        ...
        <table>
            ...
            <tr class="details">
                ...
                @if($data['delivered']!=0)
                <td>{{ round($data['total_livraison']/$data['delivered'], 2)}} Dhs</td>
                @else
                    <td>0 Dhs </td>
                    @endif
            ...
            <tr class="total" style="border-top: 1px solid #777;">
                ...
                <td colspan="2">{{round($data['total_delivered'],2)}} Dhs</td>
            </tr>
            <tr class="total">
                <td colspan="3" class='total-text'>Frais de livraison:</td>
                <td colspan="2">{{round($data['total_livraison']+$data['total_canceled']+$data['total_returned'],2)}} Dhs</td>
            </tr>
            <tr class="total">
                <td colspan="3" class='total-text'>Total Net:</td>
                <td colspan="2">{{round($data['total_delivered']-($data['total_livraison']+$data['total_returned']+$data['total_canceled']),2)}} Dhs</td>
            </tr>
        </table>
        <table>
            <tr class="heading">
            ...
            @if(isset($commande) && $key->id_commande==$commande )
            <tr class="item" style="background: #ffc107 !important;">
            @else
            <tr class="item">
            @endif
            ...
            <td>{{ $key->id_commande }}</td>
                ...
                <td>{{ round($key->prix_livraison_final,2) }} Dhs</td>
                @elseif($key->etat_commande=='RETURNEDAG' || $key->etat_commande=='RETURNEDEV')
                <td>{{ round($key->prix_retour,2) }} Dhs</td>
                @elseif($key->etat_commande=='CANCELED')
                <td>{{ round($key->prix_refus,2) }} Dhs</td>
                @endif
